 ...
 *
 */
public class ToppyPageRanker {

	// Node class for storing data of each node
	public class Node {
		int pageId;
		double pageRank;
		List<Integer> inLinks; // List of documents that have links to this page
		List<Integer> outLinks; // List of documents that have links from this page
	...
	public void initialize() {
		sinkNodes = new ArrayList<Integer>();
	...
	public boolean isConverge() {
		...
		for (int i = perplexity.size() - 1; i > perplexity.size() - 4; i--) {
			...
			if (Math.floor(p1) != Math.floor(p2)) return false; // Second cond. prevents rare case of same unit with far values ex. 101, 91, 71, 31
		}
		return true;
	}
